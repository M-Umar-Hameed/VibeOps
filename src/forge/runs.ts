...
async function reviewStage(
  ...
  const reviewRes = await track(actorId, ticket.id, "review", run.agents.review, () => runAgent(
    reviewAgent,
    composeReviewPrompt({ ticket, plan, report: reportOutput, diff: reviewDiffPayload(diff, stat), operatorNotes: run.operatorNotes, checks: checksText, protectedViolation: protectedFinding, amendments, gate: gate.report || undefined }) + roleStyle("review", styleSetting),
    workdir, onData,
    (child) => { run.child = child; },
  ));
  run.child = undefined;
