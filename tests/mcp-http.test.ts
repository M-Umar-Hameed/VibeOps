...
test("MCP over HTTP: 401 keyless, tools listed with key, config + install endpoints", { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    expect(names).toEqual(["browser_act", "browser_marks", "browser_read", "browser_snapshot", "browser_tabs", "comment", "create_ticket", "delete_note", "fetch_docs", "get_ticket_history", "list_notes", "save_decision", "save_note", "save_rule", "search_knowledge", "search_tickets", "update_note", "update_ticket"].sort());
    await client.close();
