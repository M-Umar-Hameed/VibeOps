...
- [ ] **Step 1b: Backup & restore (survive a machine reinstall).** Document that `~/.vibeops` is the SINGLE backup unit: copying the folder = full backup (database + credentials); restoring it onto a fresh machine/Windows reinstall BEFORE first run = full restore (boot detects existing actors, skips bootstrap, app auto-detects the restored credentials). Motivated by real loss: a Windows reinstall previously wiped a machine-local (claude-mem) memory setup entirely. No backup tooling is built (ponytail — copy a folder); the design just guarantees the folder IS the state.
...
Problem: work done in Gemini/Codex/Antigravity/local LLMs is invisible to later sessions in other tools. claude-mem's automatic capture is Claude-Code-only (its store IS reachable over MCP/HTTP at :37777, so other tools can be hand-wired to it — the user did this once — but that setup is machine-local plugin state and was completely lost in a Windows reinstall). Solution direction: a session-transcript ingestion connector in the existing knowledge layer — tail each tool's local session logs (Claude Code `~/.claude/projects/**/*.jsonl`, Codex `~/.codex/sessions`, Gemini CLI `~/.gemini`, Antigravity, local-LLM chat exports) AND, where present, read claude-mem's observations store as one more source (already-summarized, structured). Parse per-tool, chunk + embed into `embeddings` under a new `session` sourceKind (schema enum extension), hash-gated like the vault watcher, citations = tool + session file. Every MCP-connected tool then shares one queryable cross-tool history via `search_knowledge`, stored in the durable VibeOps DB (backed up by copying `.vibeops` / a real Postgres) and REBUILDABLE after a machine reinstall from whatever logs survive. Per-tool parsers are the per-connector work; log locations/formats must be verified per tool at design time.
