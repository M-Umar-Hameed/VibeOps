...
test.skipIf(EMBEDDED)("saveNote writes note + audit event + embedding", async () => {
...
test.skipIf(EMBEDDED)("embedding failure keeps the note un-indexed, then sweep fixes it", async () => {
...
test.skipIf(EMBEDDED)("project-scoped note with bad refId throws NotFoundError", async () => {
