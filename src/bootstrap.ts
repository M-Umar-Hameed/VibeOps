...
export async function runBootstrap(
  port: number, dir = join(vibeopsHome(), ".vibeops"),
  ...
  // Never clobber an existing owner credential, even when the actors table is
  // empty (DB reset/restore, or a throwaway test DB). Overwriting destroys the
  // key the desktop app / extension / scripts already hold.
  const credsPath = join(dir, "credentials.json");
  if (existsSync(credsPath)) return { bootstrapped: false };
